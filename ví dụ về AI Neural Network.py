...
class NeuralNetwork(object): #OOP related
    ...
    def feedForward(self, x): #hàm cho ra kết quả tiếp theo
        for b, w in zip(self.biases, self.weights): #b là hệ số bias, w là hệ số weight
            z = np.dot(w, a) + b
            x = sigmoid(z) #chạy qua 1 layer neuron
    ...
    def evaluate(self, testData):
        ...
        return sum(int(x == y) for (x, y) in test results) #nếu mà x == y thì sẽ +1 lần, sum sẽ là tổng số lần chính xác
    ...
    def sigmoidDer(z):
        return sigmoid(z)*(1-sigmoid(z)) #Đạo hàm của hàm phi tuyến